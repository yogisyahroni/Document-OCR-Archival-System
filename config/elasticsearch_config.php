...
return [
    ...
    */
    ...
    'connection_pool' => [
        'class' => $_ENV['ELASTICSEARCH_CONNECTION_POOL'] ?? '\\Elasticsearch\\ConnectionPool\\StaticNoPingConnectionPool',
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | OCR Result Index Configuration
    |--------------------------------------------------------------------------
    |
    | These settings specifically configure the index for storing OCR results
    | and extracted document numbers for fast lookup.
    |
    */

    'ocr_results_index' => [
        'index' => $_ENV['OCR_RESULTS_INDEX_NAME'] ?? 'ocr-results',
        'settings' => [
            'number_of_shards' => (int) ($_ENV['OCR_RESULTS_SHARDS'] ?? 1),
            'number_of_replicas' => (int) ($_ENV['OCR_RESULTS_REPLICAS'] ?? 0),
            'refresh_interval' => $_ENV['OCR_RESULTS_REFRESH_INTERVAL'] ?? '1s',
            'index.max_result_window' => (int) ($_ENV['OCR_RESULTS_MAX_RESULT_WINDOW'] ?? 10000),
        ],
        'mappings' => [
            'properties' => [
                'document_id' => [
